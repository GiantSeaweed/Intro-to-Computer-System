...
inline uint32_t internal_normalize(uint32_t sign, int32_t exp, uint64_t sig_grs) {
	...
	if((sig_grs >> (23 + 3)) > 1 || exp < 0) {
		...
		while((((sig_grs >> (23 + 3)) > 1) && exp < 0xff) // condition 1
			...
			sig_grs = sig_grs >> 1;
			sig_grs |= sticky;//sticky is the record the passed bits
			exp++;
		}
...
//	printf("final sign=%x exp=%x frac=%x f.val=%x\n",f.sign,f.exponent,f.fraction,f.val);
	return f.val;
}
...
uint32_t internal_float_add(uint32_t b, uint32_t a) {
	...
	sig_a = fa.fraction;// printf("presig_a=%x\n",sig_a);
	if(fa.exponent != 0) sig_a |= 0x800000; // the hidden 1
	sig_b = fb.fraction;// printf("presig_b=%x\n",sig_b);
	...
	/* TODO: shift = ? */
	uint32_t fb_exp_tmp=fb.exponent;
	uint32_t fa_exp_tmp=fa.exponent;
	if(fa.exponent==0) {
			fa_exp_tmp=fa.exponent+1;
	}
	if(fb.exponent==0){
			fb_exp_tmp=fb.exponent+1;
	}
	shift=fb_exp_tmp-fa_exp_tmp;
//	printf("fa_exp_ori=%x,fb_exp_ori=%x\n",fa.exponent,fb.exponent);
	//printf("\e[0;31mPlease implement me at fpu.c\e[0m\n");
	//assert(0);
	assert(shift >= 0);
...
//	printf("shift=%d\n",shift);
	...
	while(shift > 0) {
			...
			sig_a |= sticky;//sticky is the record the passed bits
			shift --;
	}
	...
	if(fa.sign) { sig_a *= -1; }
	if(fb.sign) { sig_b *= -1; }
//	printf("fa.sign=%x fb.sign=%x\n",fa.sign,fb.sign);
	sig_res = sig_a + sig_b;
//	printf("sig_res=%x\n",sig_res);
	if(sign(sig_res)){ f.sign = 1; sig_res *= -1; }//sig_res is positive
	else { f.sign = 0; }

	uint32_t exp_res = fb.exponent;
//	if((sig_res<(0x800000<<3)) && fb.exponent==0){
//	if((sig_res<(0x800000<<3)) ){
	if(sig_res==0){
			exp_res=0;
	}
//	printf("sign=%x exp=%x sig_res=%x\n",f.sign,exp_res,sig_res);
